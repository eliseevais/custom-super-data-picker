import { format, isAfter, isValid, parseISO, sub, differenceInSeconds, startOfDay, startOfWeek, startOfMonth, startOfYear } from 'date-fns';
...
export const parseDateString = (dateString: string): Date => {
  ...
  if (dateString.startsWith('now/')) {
    ...
    switch (unit) {
      case 'd': return startOfDay(now);
      case 'w': return startOfWeek(now, { weekStartsOn: 1 }); // понедельник
      case 'M': return startOfMonth(now);
      case 'y': return startOfYear(now);
      default:
  ...
  if (dateString.includes('/')) {
    ...
    switch (truncation) {
      case 'd': return startOfDay(baseDate);
      case 'w': return startOfWeek(baseDate, { weekStartsOn: 1 });
      case 'M': return startOfMonth(baseDate);
      case 'y': return startOfYear(baseDate);
      default:
